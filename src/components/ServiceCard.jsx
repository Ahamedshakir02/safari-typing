...
export default function ServiceCard({ service, variant = 'grid' }) {
  ...
  return (
    <Link
      ...
      className={`group relative flex h-full flex-col rounded-2xl border border-navy/10 bg-white shadow-card transition-all duration-300 hover:-translate-y-1 hover:border-navy/20 hover:shadow-cardHover ${
        compact ? 'p-5' : 'p-6'
      }`}
    >
