 ...
 * page scroll (pauses Lenis), and restores focus to the trigger on close.
 */
export default function MobileMenu({ open, onClose }) {
  ...
  return (
    <div
      ref={root}
      className="fixed inset-0 z-[130] bg-navy text-offwhite md:hidden"
      role="dialog"
      aria-modal="true"
      aria-label="Menu"
    >
      <div ref={panel} className="flex h-full flex-col px-6 pb-8 pt-5">
        ...
        <nav className="mt-10 flex flex-1 flex-col gap-1" aria-label="Primary">
          {NAV_LINKS.map((link) => (
            <SmartLink
              ...
              to={link.to}
              onNavigate={onClose}
              className="menu-item border-b border-white/10 py-4 font-display text-3xl font-medium tracking-tight"
            >
              {link.label}
            </SmartLink>
          ))}
        ...
        <div className="menu-item mt-6 grid grid-cols-2 gap-3">
          <a
            href={waLink()}
            target="_blank"
        ...
        <Arabic className="menu-item mt-6 block text-center text-sm text-offwhite/60">
